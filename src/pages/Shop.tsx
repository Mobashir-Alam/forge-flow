    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-1">
          ...
          {categories.map(c => (
            <button key={c.name} onClick={() => setSelectedCategory(c.name)} className={`block w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${selectedCategory === c.name ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-secondary"}`}>
              {c.icon} {c.name} <span className="text-muted-foreground/60">({c.count})</span>
            </button>
          ))}
        </div>
      </div>
